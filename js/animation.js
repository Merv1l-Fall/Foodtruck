export function animateToCart(buttonElement) {
    ...
    requestAnimationFrame(() => {
        ...
        const translateX = cartRect.left - itemRect.left + (cartButton.offsetWidth / 2 - itemRect.width / 2);
		const translateY = cartRect.top - itemRect.top + (cartButton.offsetHeight / 2 - itemRect.height / 2);

        clone.style.transform = `translate(${translateX}px, ${translateY}px) scale(0.3)`;
        clone.style.transition = 'transform 0.5s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.5s';
